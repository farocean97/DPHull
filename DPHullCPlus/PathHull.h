...
public:
	...
	const typename TPointContainer::const_iterator& GetpElt(int i)	const	{	ASSERT(i<m_ppElt.size()); return m_ppElt[i];};
	const typename TPointContainer::const_iterator& GetpHelt(int i)	const	{	ASSERT(i<m_ppHelt.size()); return m_ppHelt[i];};
	...
	void SetTopElt(const typename TPointContainer::const_iterator& p)		{	ASSERT(m_iTop>=0); ASSERT(m_iTop<m_ppElt.size()); m_ppElt[m_iTop]=p;};
	void SetBotElt(const typename TPointContainer::const_iterator& p)		{	ASSERT(m_iBot>=0); ASSERT(m_iBot<m_ppElt.size()); m_ppElt[m_iBot]=p;};

	void Split(const typename TPointContainer::const_iterator& e)
	{
	...
	void FindExtreme(const typename TLine<T,TPointContainer,TKeyContainer>::SHomog& line, typename TPointContainer::const_iterator* e, T& dist)
	{
	...
	void Init(const typename TPointContainer::const_iterator& e1, const typename TPointContainer::const_iterator& e2)
	{
	...
	void Push(const typename TPointContainer::const_iterator& e)
	{
		ASSERT(m_iTop+1 >= 0);
		ASSERT(m_iTop+1 < m_ppElt.size());
		ASSERT(m_iBot-1 >= 0);
	...
	void Add(const typename TPointContainer::const_iterator& p)
	{
		int topflag, botflag;
	...
	int LeftOfTop(const typename TPointContainer::const_iterator& c)
	{
		ASSERT(m_iTop >= 1);
		ASSERT(m_iTop < m_ppElt.size());
	...
	int LeftOfBot(const typename TPointContainer::const_iterator& c)
	{
		ASSERT(m_iBot >= 0);
		ASSERT(m_iBot+1 < m_ppElt.size());
	...
	int SlopeSign(int p, int q, const typename TLine<T,TPointContainer,TKeyContainer>::SHomog& l)
	{
		ASSERT(p >= 0);
